...
inline void init_lib()
{
	if (!initialized)
	{
		...
		if (!bthprops_lib)
		{
			throw "Failed to load bthprops.cpl";
			exit(EXIT_FAILURE);
		}
...
int FindWiimotes(Wiimote** wm, int max_wiimotes)
{
	...
	for (int index = 0; found_wiimotes < max_wiimotes; ++index)
	{
		...
		if (!wm[k]->Connect())
		{
			LOG("Unable to connect to wiimote ", index + 1);
			delete wm[k];
			wm[k] = NULL;
			CloseHandle(dev);
		}
...
bool Wiimote::Connect()
{
	...
	{
		LOG("Failed to set wiimote thread priority");
	}

	LOG("Connected to wiimote ", index + 1);
	...
	*buf = 0;
	if (!ReadFile(dev_handle, buf, MAX_PAYLOAD, &b, &hid_overlap))
	{
		...
		if (r == WAIT_TIMEOUT)
		{
			...
			if (*buf)
				LOG("Packet ignored.  This may indicate a problem (timeout is %i ms).");
		...
		{
			LOG("A wait error occured on reading from wiimote %i.");
			return 0;
		}
...
int Wiimote::IOWrite(unsigned char* buf, int len)
{
	...
	switch (stack)
	{
		case MSBT_STACK_UNKNOWN:
			{
				...
				if (dw == 121)
				{
					LOG("IOWrite[MSBT_STACK_UNKNOWN]: Timeout");
					RealDisconnect();
				}
        else LOG("IOWrite[MSBT_STACK_UNKNOWN]: ERROR: ", to_hex(dw)); 
				return 0;
			}
		...
		case MSBT_STACK_MS:
			...
			{
				// Semaphore timeout
				LOG("WiimoteIOWrite[MSBT_STACK_MS]:  Unable to send data to wiimote");
				RealDisconnect();
				return 0;
			}
			return i;
...
int PairUp(bool unpair)
{
	...
	while (hFindRadio)
	{
		...
		while (hFindDevice)
		{
			...
			LOG("authed ", btdi.fAuthenticated, " connected ", btdi.fConnected, " remembered ", btdi.fRemembered);
			...
			if (std::regex_match(btdi.szName, wiimote_device_name))
			{
				if (unpair)
				{
					...
					{
						LOG("Pair-Up: Automatically removed BT Device on shutdown: %08x");
						++nPaired;
					}
				}
				else
				{
					if (false == btdi.fConnected)
					{
						...
						if (SUCCEEDED(hr))
							...
							LOG("Pair-Up: BluetoothSetServiceState() returned %08x");
					}
				}
			}
