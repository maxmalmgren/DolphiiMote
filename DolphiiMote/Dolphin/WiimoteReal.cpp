...
std::vector<std::function<void(int, u16, const void*, u32)>> callbacks;
...
void AddCallback(std::function<void(int, u16, const void*, u32)> callback)
{
  std::unique_lock<std::mutex> lock(callback_lock);
  callbacks.push_back(callback);
}

void NotifyCallbacks(int _number, u16 _channelID, const void* _pData, u32 _Size)
{
  std::unique_lock<std::mutex> lock(callback_lock);

  for(auto& callback : callbacks)
    callback(_number, _channelID, _pData, _Size);
}
...
void Wiimote::ControlChannel(const u16 channel, const void* const data, const u32 size)
{
	...
	else
	{
      ...
      NotifyCallbacks(index, channel, &handshake_ok, sizeof(handshake_ok));
		}
	}
}
...
void Wiimote::Update()
{
  ...
  {
    NotifyCallbacks(index, m_channel, rpt.first, rpt.second);
  }
...
bool Wiimote::Handshake()
{
	...
	LOG("Sending handshake to wiimote");
...
unsigned int Initialize()
{
  ...
  LOG("Found ", g_wiimotes_found, " Real Wiimotes, ", wanted_wiimotes, " wanted");
  ...
  LOG("Connected to ", g_wiimotes_found, " Real Wiimotes");
...
void Refresh()
{
	...
	if (wanted_wiimotes > g_wiimotes_found)
	{
		...
		LOG(WIIMOTE, "Found %i Real Wiimotes, %i wanted", num_wiimotes, wanted_wiimotes);

		// Connect newly found wiimotes.
		int num_new_wiimotes = ConnectWiimotes(g_wiimotes);

		LOG(WIIMOTE, "Connected to %i additional Real Wiimotes", num_new_wiimotes);
